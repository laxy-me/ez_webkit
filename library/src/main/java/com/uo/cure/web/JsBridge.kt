...
import com.uo.cure.tools.MethodUtil
import com.uo.cure.tools.Preference
...
open class JsBridge(private val mContext: Context) {
    ...
    fun openPureBrowser(json: String) {
        ...
        if (mContext is WebActivity) {
            ...
            Launcher.with(mContext, PureWebActivity::class.java)
                .putExtra(ExtraKeys.EX_STATE_BAR_BG, jsonObject.optString("titleColor"))
                .putExtra(
                    ExtraKeys.EX_STATE_BAR_FIELD_COLOR,
                    jsonObject.optString("stateBarTextColor")
                )
                .putExtra(
                    ExtraKeys.EX_TITLE_TEXT_COLOR,
                    jsonObject.optString("titleTextColor")
                )
                .putExtra(ExtraKeys.EX_TITLE, jsonObject.optString("title"))
                .putExtra(ExtraKeys.EX_HAS_TITLE_BAR, jsonObject.optString("hasTitleBar"))
                .putExtra(ExtraKeys.EX_REWRITE_TITLE, jsonObject.optString("rewriteTitle"))
                .putExtra(ExtraKeys.EX_URL, jsonObject.optString("url"))
                .putExtra(ExtraKeys.EX_POST_DATA, jsonObject.optString("postData"))
                .putExtra(ExtraKeys.EX_HTML, jsonObject.optString("html"))
                .putExtra(ExtraKeys.EX_WEB_BACK, jsonObject.optString("webBack"))
                .execute()
        }
    }
    ...
    fun newPageWithTitleBar(url: String, title: String?, hasTitleBar: Boolean) {
        if (mContext is WebActivity) {
            ...
            title?.let {
                launcher.putExtra(ExtraKeys.EX_TITLE, title)
            }
            when {
                else -> launcher.putExtra(ExtraKeys.EX_URL, url)
                    .putExtra(ExtraKeys.EX_HAS_TITLE_BAR, hasTitleBar)
                    .execute()
            }
        }
    }
