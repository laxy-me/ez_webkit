        ...
        try {
            launch {
                ...
                if (response.isNotBlank()) {
                    val code = JSONObject(response).optInt("code")
                    val googleToken = JSONObject(response).optJSONObject("data")
                    googleToken?.let {
                        val rawCookie: String = createTokenStr(
                            "token1", googleToken.optString("token1")!!
                        ) + "\n" + createTokenStr("token2", googleToken.optString("token2"))
                        if (!TextUtils.isEmpty(rawCookie) && !TextUtils.isEmpty(host)) {
                            ...
                            if (act is WebActivity) {
                                act.getWebView()
                                    .loadUrl(URLDecoder.decode(googleToken.optString("url")))
                            }
                        }
                    }
                }
